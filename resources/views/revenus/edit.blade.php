...
@section('Content')
    ...
    <div class="px-4 py-5 my-1 text-center">
        <h1 class="display-5 fw-bold">{{__('revenu.editInfo')}}</h1>
                    ...
                    @foreach(config('locales.languages') as $key => $val)
                        <div class="tab-pane fade {{ $loop->index == 0 ? 'show active' : '' }}" id="{{ $key }}" role="tabpanel" aria-labelledby="{{ $key }}-tab" tabindex="0">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="libelle[{{ $key }}]"  value="{{old('libelle.' . $key, $revenu->getTranslation('libelle', $key))}}">
                            </div>
                            <div class="mb-3">
                                <label for="source" class="form-label">{{__('revenu.source')}}</label>
                                <input type="text"  class="form-control" name="source[{{ $key }}]" value="{{old('source.' . $key, $revenu->getTranslation('source', $key))}}"  >
                            </div>
