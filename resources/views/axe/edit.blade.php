...
@section('Content')
    ...
    <div class="px-4 py-5 my-1 text-center">
        <h1 class="display-5 fw-bold">{{__('axes.edit')}}</h1>
    ...
    <div class="container">
        ...
        @endif
        <form action="{{route('Axe/update',$axes->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('POST')
            <nav>
                ...
                @foreach(config('locales.languages') as $key => $val)
                    <div class="tab-pane fade {{ $loop->index == 0 ? 'show active' : '' }}" id="{{ $key }}" role="tabpanel" aria-labelledby="{{ $key }}-tab" tabindex="0">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control"name="nom[{{ $key }}]" value="{{old('nom.' . $key, $axes->getTranslation('nom', $key)) }}" >
                        </div>
                        <br>
                    </div>
                @endforeach
            ...
            <div class="form-group">
                <button type="submit" name="submit" class="btn btn-primary">{{ __('global.add') }}</button>
            </div>
        </form>
    </div>
